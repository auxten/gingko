...
int conn_tcp_server(struct conn_server *c)
{
	...
	// Bind socket
	if (bind(server->listen_fd, (struct sockaddr *) &server->listen_addr, sizeof(server->listen_addr)) < 0)
	{
	...
	// Set socket options
	struct timeval timeout = {tv_sec: server->tcp_timeout, tv_usec: 0};

	setsockopt(server->listen_fd, SOL_SOCKET, SO_REUSEADDR, &server->tcp_reuse, sizeof(server->tcp_reuse));
	setsockopt(server->listen_fd, SOL_SOCKET, SO_SNDTIMEO, (char *) &timeout, sizeof(struct timeval));
	setsockopt(server->listen_fd, SOL_SOCKET, SO_SNDBUF, &server->tcp_send_buffer_size, sizeof(server->tcp_send_buffer_size));
	setsockopt(server->listen_fd, SOL_SOCKET, SO_RCVBUF, &server->tcp_recv_buffer_size, sizeof(server->tcp_recv_buffer_size));
	setsockopt(server->listen_fd, IPPROTO_TCP, TCP_NODELAY, (char *) &server->tcp_nodelay, sizeof(server->tcp_nodelay));
	...
	//ev_base = event_init();
	// Add data handler
	event_set(&server->ev_accept, server->listen_fd, EV_READ|EV_PERSIST, conn_tcp_server_accept, (void *) c);
...
void conn_tcp_server_on_data(int fd, short ev, void *arg)
{
	...
	while(1)
	{
		res = read(client->client_fd, client->read_buffer, buffer_avail);
		//printf("res:%d\n", res);

		if (res > 0 && ((unsigned int) res == buffer_avail))
		{
		...
		if ( res == 0 || res > 0)
		{
			...
			{
				server->on_data_callback(client->id, (void *) client->read_buffer, sizeof(client->read_buffer));
			}
			conn_client_free(client);
			break;
		}
...
struct conn_client * add_new_conn_client(int client_fd)
{
	...
	{
		//Client list pool full, if you want to enlarge it, modify conn.h source please
