...
host_hash_result * host_hash(s_job * jo, const s_host * new_host,
	...
	u_int64_t host_key = (((u_int64_t) inet_addr(
			new_host->addr)) << 32) + (new_host->port) << 16;
	u_int64_t vnode_distance =
			(jo->block_count / VNODE_NUM / 3) ? ((u_int64_t)fnv_hash((void *) new_host,
					sizeof(s_host), 0) % (jo->block_count / VNODE_NUM / 3)
					+ (jo->block_count * 2 / VNODE_NUM / 3)) : 1;
	printf("fnv_hash: %lld, ip: %s, port: %d\n", (u_int64_t)fnv_hash((void *) new_host, sizeof(s_host), 0), new_host->addr, new_host->port);
	printf("fnv_hash: %lld, ip: %s, port: %d\n", (u_int64_t)fnv_hash((void *) new_host, sizeof(s_host), 0), new_host->addr, new_host->port);
	u_int64_t vnode_start = host_key % (jo->block_count);
	printf("vnode_distance:%lld, vnode_start:%lld\n", vnode_distance, vnode_start);
	for (int i = 0; i < VNODE_NUM; i++) {
		int64_t vnode = (i * vnode_distance + vnode_start)
				% (jo->block_count);
...
int64_t getblock(s_host * dhost, int64_t num, int64_t count, unsigned char flag, char * buf) {
	int sock;
	int64_t i,j;
	char msg[MSG_LEN];
	char * arg_array[4];
...
int vnode_download(s_job * jo, int64_t blk_num, int64_t blk_count) {
	...
	while (blk_got < blk_count && retry <= MAX_RETRY) {
		...
		i = decide_src(jo, 3, (blk_num+blk_got)%jo->block_count, &h_vec, &h);
		if (i <= 0) {
			retry ++;
		...
		if (h == the_server) {
			...
			while (!((job.blocks + (blk_num + count2edge) % job.block_count)->host_set)
				|| (*((job.blocks + (blk_num + count2edge) % job.block_count)->host_set)).size()
							== 0) {
				count2edge++;
			}
			pthread_rwlock_unlock(&blk_hostset_lock);
...
void * downloadworker(void *) {
	...
	for (vector<s_host>::iterator it = hosts_noready.begin();
			it != hosts_noready.end(); it++) {
