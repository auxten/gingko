...
GKO_STATIC_FUNC void * get_blocks_s(void * uri, int fd)
{
...
#endif /** GINGKO_SERV **/
    block_have = (i + 1) / 2;
    msg_len = snprintf(msg, MSG_LEN, "%sHAVE\t%lld", PREFIX_CMD, block_have);
    ...
    if (block_have && (i = sendblocks_zip(fd, jo, start % jo->block_count, block_have)) < 0)
    {
        gko_log(DEBUG, "sendblocks error have %lld!", block_have);
        return (void *) -1;
    }
    return (void *) 0;
}
...
{
    char hi_msg[SHORT_MSG_LEN] = {'\0'};
    int msg_len;

    msg_len = snprintf(hi_msg, sizeof(hi_msg), "%sHI", PREFIX_CMD);
    if (msg_len <= CMD_PREFIX_BYTE)
    {
        gko_log(WARNING, FLF("snprintf error"));
        return (void *) -1;
    ...
    {
        fill_cmd_head(hi_msg, msg_len);
    }
    if (sendall(fd, hi_msg, msg_len, SND_TIMEOUT) < 0)
    {
...
GKO_STATIC_FUNC void * join_job_s(void * uri, int fd)
{
    ...
    /**
     * check if the seed is timeout
     */
    struct timeval now;
    struct timeval seed_age;
    struct stat file_stat;
    char is_file_modified = 0;
    char is_seed_old = 0;

    pthread_mutex_lock(&g_job_lock[jo->lock_id].lock);
    gettimeofday(&now, NULL);
    timersub(&now, &jo->dl_time, &seed_age);
    if (seed_age.tv_sec >= SEED_CHECK_TIME)
    {
        is_seed_old = 1;
    }
    ...
    {
        if (stat((jo->files + i)->name, &file_stat))
        {
            gko_log(WARNING, "cant stat %s", (jo->files + i)->name);
            is_file_modified = 1;
            break;
        }
    ...
    gko_log(DEBUG, "percent: %d, %d", percent, seed_age.tv_sec);
    if (percent == 100 &&
        is_seed_old &&
        jo->file_count != 0 &&
        jo->total_size != 0 &&
        jo->dl_time.tv_sec != 0 &&
        is_file_modified)
        /// dl_time is the seed's birthday
    {
        gko_log(WARNING, FLF("seed is out of date!"));
    ...
    int i_a = 0;
    for (std::set<s_host_t>::const_iterator i = (*(jo->host_set)).begin(); i
            != (*(jo->host_set)).end(); i++)
    {
