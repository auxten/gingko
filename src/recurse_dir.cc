...
int init_struct(const char *name, const struct stat *status, int type,
    ...
    if (strcmp(".", name) != 0) {
        ...
        if (type == FTW_D) {
            ...
            if (-1 == readlink(name,
                    (p_dir->files + p_dir->init_s_file_iter)->sympath,
                    MAX_PATH_LEN)) {
        ...
        memcpy((p_dir->files + p_dir->init_s_file_iter)->name, name,
                strlen(name) + 1);
...
int recurse_dir(s_job * jo) {
    ...
    if (p_dir->total_size)
        p_dir->blocks = (s_block *) calloc(BLOCK_COUNT(p_dir->total_size),
                sizeof(s_block));
    ...
    if (p_dir->total_size) {
        (p_dir->blocks + blk_cnt)->size = BLOCK_SIZE - ((blk_cnt + 1)
                * BLOCK_SIZE - p_dir->total_size);
        for (j = p_dir->file_count - 1, i = p_dir->total_size; i
                - (((p_dir->files + j)->size > 0) ? ((p_dir->files + j)->size)
                        : 0) > blk_cnt * BLOCK_SIZE && j >= 0; i
                -= ((p_dir->files + j)->size > 0) ? ((p_dir->files + j)->size)
                        : 0, j--) {
            //printf("start_f: %d, off: %d\n", j, i);
        }
        if (((p_dir->blocks + blk_cnt)->start_f != j) || ((p_dir->blocks
                + blk_cnt)->start_off != (blk_cnt) * BLOCK_SIZE - (i
                - (p_dir->files + j)->size))) {
    ...
    for (i = 0; i < p_dir->file_count; i++) {
        printf("0%3o\t\t%lld\t\t\t%s\t%s\t", (p_dir->files + i)->mode & 0777,
                (p_dir->files + i)->size, (p_dir->files + i)->name,
                (p_dir->files + i)->sympath);
        for (j = 0; j < 16; j++)
            printf("%02x", (p_dir->files + i)->md5[j]);
        printf("\n");
