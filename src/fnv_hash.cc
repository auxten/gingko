...
unsigned fnv_hash_file(unsigned value, FILE * fd, off_t * off, size_t * count,unsigned char * buf ) {
...
void * fnv_hash_worker_f(void * a) {
	...
	while (total > 0) {
		...
		if (block_left > file_left) {
			(jo->blocks + b)->digest =
				fnv_hash_file((jo->blocks + b)->digest, fd, &offset, &file_left, buf);
			fclose(fd);
			fd = (FILE *)-1;
			...
			(jo->blocks + b)->digest =
				fnv_hash_file((jo->blocks + b)->digest, fd, &offset, &block_left, buf);
			b = next_b(jo, b);
			total -= block_left;
			offset += block_left;
			sent += block_left;
